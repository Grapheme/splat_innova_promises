		...
		<div class="normal-wrapper">
			...
			<div class="normal-text">
				<p>
                    ...
                    CSV за 7 дней:
                    <a href="{{ URL::route('app.statistics_promises_all', ['format' => 'csv', 'days' => 7, 'bom' => 1, 'quotes' => 1, 'comma' => ',']) }}">с BOM"</a>,
                    <a href="{{ URL::route('app.statistics_promises_all', ['format' => 'csv', 'days' => 7, 'bom' => 0, 'quotes' => 1, 'comma' => ',']) }}">без BOM"</a>,
                    <a href="{{ URL::route('app.statistics_promises_all', ['format' => 'csv', 'days' => 7, 'bom' => 1, 'quotes' => 0, 'comma' => ',']) }}">с BOM</a>,
                    <a href="{{ URL::route('app.statistics_promises_all', ['format' => 'csv', 'days' => 7, 'bom' => 0, 'quotes' => 0, 'comma' => ',']) }}">без BOM</a>
                    ).
				</p>
						...
						@foreach($expired_promises as $promise)
							...
							<li>
								...
								@if (isset($expired_promises_users[$promise->user_id]) && $expired_promises_users[$promise->user_id]->auth_method != 'native')
									<a href="{{ $expired_promises_users[$promise->user_id]->identity }}" target="_blank"><i class="fa fa-user"></i></a>
								@endif
